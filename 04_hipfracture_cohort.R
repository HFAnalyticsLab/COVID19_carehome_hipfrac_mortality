...
(apcc_hip %>% 
    ...
    theme(legend.title = element_blank(),
          ...
          axis.text.x = element_text(angle = -45, hjust = 0, vjust = 1)) +
    labs(title = "Hip fracture admissions", x= "", y = "Admissions")) %>% 
  ggsave(str_c(results_path, "counts/hip_admissions_residence_monthly.png"), ., device = "png", 
         dpi = 600, width = 6, height = 3.7)
...
(admission_roll %>% 
   ggplot(aes(x = cipsstartdate_dummy, y = admissions_7dayavg, group = year_adm, color = year_adm)) +
   geom_line() +
   scale_x_date(date_labels = "%b", date_breaks = "1 month") +
   theme_bw() +
   expand_limits(y=0)+
   theme(legend.title = element_blank(),
         ...
         panel.grid.minor = element_blank(),
         axis.text.x = element_text(angle = -45, hjust = 0, vjust = 1)) +
   labs(title = "Hip fracture admissions\n(7-day average)", x= "", y = "Admissions")) %>% 
  ggsave(str_c(results_path, "counts/hip_admissions_all_7dayavg.png"), ., device = "png", 
         dpi = 600, width = 3, height = 4)

admission_residence_roll <- apcc_hip %>%
  group_by(year_adm, residence, cipsstartdate) %>% 
  arrange(cipsstartdate) %>% 
  summarise(admissions = n()) %>% 
  mutate(admissions_7dayavg = sevendayavg(admissions),
         cipsstartdate_dummy = if_else(month(cipsstartdate) %in% c(3:12), 
...
write_csv(admission_residence_roll, str_c(results_path, "counts/hip_admissions_residence_7dayavg.csv"))

(admission_residence_roll %>% 
    ggplot(aes(x = cipsstartdate_dummy, y = admissions_7dayavg, group = year_adm, color = year_adm)) +
    geom_line() +
    scale_x_date(date_labels = "%b", date_breaks = "1 month") +
    facet_wrap("residence", scales = "free_y") +
    ...
    theme(legend.title = element_blank(),
          ...
          axis.text.x = element_text(angle = -45, hjust = 0, vjust = 1)) +
    labs(title = "Hip fracture admissions\n(7-day average)", x= "", y = "Admissions")) %>% 
  ggsave(str_c(results_path, "counts/hip_admissions_residence_7dayavg.png"), ., device = "png", 
...
# LoS: median + IQR
los_adm_year_residence <-  apcc_hip %>% 
  filter(!is.na(year_adm)) %>% 
  group_by(year_adm, residence) %>% 
  summarise(n = n(),
            los_median = median(los),
            los_IQR = IQR(los),
            los_median_IQR = str_c(los_median, " (", los_IQR, ")"),
            los_median_n = sum(los == los_median))

write.csv(los_adm_year_residence, str_c(results_path, 'chars/hipadmissions_los_year_residence.csv'))

los_adm_year_residence_covid <-  apcc_hip %>% 
  filter(!is.na(year_adm)) %>% 
  group_by(year_adm, covid_any, residence) %>% 
  summarise(n = n(),
            los_median = median(los),
            los_IQR = IQR(los),
            los_median_IQR = str_c(los_median, " (", los_IQR, ")"),
            # Number of patients with median as LoS
            los_median_n = sum(los == los_median),
            # If LoS is average, number of patients used to calculate average
            los_median_n_tiebreak = sum(los == floor(los_median) + sum(los == ceiling(los_median)))) %>% 
  filter(year_adm == "2020/21")

write.csv(los_adm_year_residence_covid, str_c(results_path, 'chars/hipadmissions_los_year_residence_covid.csv'))
