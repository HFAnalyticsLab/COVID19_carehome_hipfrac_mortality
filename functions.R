...
count_days_covered <- function(period_start, period_end){
  map2_dbl(period_start, period_end, 
           ~ ifelse(is.na(.x) | is.na(.y) | .y < .x, 0, length(seq.Date(from = .x, to = .y, by = "day")))
  )
}
...
count_days_missing <- function(period1_end, period2_start){
  map2_dbl(period1_end, period2_start, 
           ~ ifelse(is.na(.x) | is.na(.y) | .y <= .x, 0, length(seq.Date(from = .x, to = .y, by = "day")) -2)
  )
}
...
count_days_overlap <- function(period1_end, period2_start){
  map2_dbl(period1_end, period2_start, 
           ~ ifelse(is.na(.x) | is.na(.y) | .y > .x, 0, length(seq.Date(from = .y, to = .x, by = "day")))
  )
}
